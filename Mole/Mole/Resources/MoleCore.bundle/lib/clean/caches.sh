...
# Preflight Full Disk Access (FDA) check directly to avoid multiple per-folder dialogs.
check_tcc_permissions() {
    local permission_flag="$HOME/.cache/mole/permissions_granted"
    # If Full Disk Access is already granted, mark flag and return
    if ls "$HOME/Library/Safari" > /dev/null 2>&1 || ls "$HOME/Library/Suggestions" > /dev/null 2>&1; then
        ensure_user_file "$permission_flag"
        return 0
    fi

    # Non-interactive mode (e.g. background runs or GUI runner) - skip terminal prompt
    [[ -t 1 ]] || return 0
    [[ -f "$permission_flag" ]] && return 0

    echo ""
    echo -e "${BLUE}全文件夹访问权限设置 (Full Disk Access)${NC}"
    echo -e "${GRAY}Mole 需要完全磁盘访问权限以彻底清理受保护的系统缓存、残留及日志。${NC}"
    echo -e "${GRAY}直接开启【完全磁盘访问权限】可免去反复弹出多个文件夹授权提示。${NC}"
    echo ""
    echo -ne "${PURPLE}${ICON_ARROW}${NC} 按 ${GREEN}Enter${NC} 自动打开系统设置授权页面: "
    read -r || true
    # Touch protected path to trigger TCC registration
    ls "$HOME/Library/Safari" > /dev/null 2>&1 || true
    open "x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles" 2>/dev/null || true
    echo -e "${GREEN}✓ 已打开系统设置【完全磁盘访问权限】。请开启 Mole 权限后继续。${NC}"
    echo ""
    ensure_user_file "$permission_flag"
    return 0
}
